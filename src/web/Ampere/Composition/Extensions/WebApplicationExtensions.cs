...
public static class WebApplicationExtensions
{
    extension(WebApplication app)
    {
        public async Task RunAmpereAsync<T>() where T : Microsoft.AspNetCore.Components.IComponent
        {
            ...
            {
                app.MapOpenApi("ampere/{v1}.json").AllowAnonymous();
                app.MapScalarApiReference("ampere/scalar",  async options =>
                {
                    options.WithOpenApiRoutePattern("/ampere/{documentName}.json");
                    options.WithTitle($"AMPERE: [{app.Environment.EnvironmentName}]");
                    options.HeadContent = @"
